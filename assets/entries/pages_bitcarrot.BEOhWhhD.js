import{j as e,f as h,n,l as c,m as i,A as a,a as t,P as f,$ as y,b as g,x as v,d as x,c as b,R as w,e as r,i as l,g as z,h as j,k as C}from"../chunks/chunk-B_U_67AP.js";import{y as k}from"../chunks/chunk-BPOV7PiO.js";/* empty css                      */const u="BSidesHBG Conference",S=[{id:"jAWQK3_7D18H_5v0pK0z4"},{id:"mONS0so-VrF1uS9rPlVcF",maxWidth:991},{id:"zGIdqUzdPde7sHmF0aRQP",maxWidth:767},{id:"wP-rcJFENR2yn86VzREAn",maxWidth:479}],_="favicon_tI45PigRlvtURPPmcaji4.ico",q=[],N=[],A=s=>e.jsx(h,{className:"w-body cmyiimz c15ybalq c1e8z8ei c8fbwtz",children:e.jsxs(n,{className:"w-box",children:[e.jsx(c,{children:e.jsxs(i,{children:[e.jsx(a,{code:`<style>
  * {
...
.result.success { background:#064e3b; }
...
<div class="main">
<div id="badgeLockBanner" style="display:none; margin-bottom:10px; color:orange; font-weight:bold;"></div>
...
<button onclick="buyCarrot()">Buy 1</button>
<button onclick="sellCarrot()">Sell 1</button>
...
window.addEventListener('DOMContentLoaded', renderBadgeLock);
let cooldownTimer = null;
// ---------------- Badge Lock ----------------
function renderBadgeLock() {
  const saved = localStorage.getItem('claimed_badge');
  const banner = document.getElementById('badgeLockBanner');
  const input = document.getElementById('badgeInput');

  if (saved) {
    banner.style.display = 'block';
    banner.textContent = '🔒 Badge Locked: ' + saved;
...
async function checkIn() {
  ...
  try {
    ...
    // CLAIM ONLY ONCE
    if (!localStorage.getItem('claimed_badge')) {

      const claimRes = await fetch(SUPABASE_URL + '/rest/v1/rpc/claim_badge', {
        method:'POST',
        headers:{
          apikey:SUPABASE_KEY,
      ...
      if (claimData.error) {
        showResult('error', claimData.error);
        return;
      }

      // lock + store
      localStorage.setItem('claimed_badge', badge);
...
async function buyCarrot(){
  ...
  const res = await fetch(SUPABASE_URL + '/rest/v1/rpc/buy_bitcarrot', {
    method:'POST',
    headers:{
      apikey:SUPABASE_KEY,
      Authorization:'Bearer '+SUPABASE_KEY,
  ...
  if (data.error) {
    ...
    showResult('success', 'Purchased 1 BitCarrot');
...
async function sellCarrot(){

  const badge = document.getElementById('badgeInput').value.trim();
  if (!badge) return;

  const res = await fetch(SUPABASE_URL + '/rest/v1/rpc/sell_bitcarrot', {
    method:'POST',
    headers:{
      apikey:SUPABASE_KEY,
      Authorization:'Bearer '+SUPABASE_KEY,
  ...
  if (data.error) {
    ...
    showResult('success', 'Sold 1 BitCarrot');
